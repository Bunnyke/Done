...
# ==== CONFIG ====
bot_token = '8039426526:AAFSqWU-fRl_gwTPqYLK8yxuS0N9at1hC4s'  # <-- Set your BotFather token
...
def get_random_proxy():
    ...
    if not good_proxies:
        # fallback: retry all if all marked bad
...
def chk(ccx, max_attempts=6):
    """Tries up to max_attempts proxies for one check; skips bad proxies live."""
    last_err = None
    for attempt in range(max_attempts):
        ...
        try:
            ...
            try:
                ...
                # Some blocks may be proxy blocks, so try a new one
                bad_proxies.add(proxy_str)
                last_err = error_msg
                continue
...
async def chk_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ccx in cc_list:
        ...
        # Fast sleep to not hammer (useful in group)
        ...
        try:
            ...
            # In case edit fails (e.g., message too old), send new
